...
class TaskDetailFragmentTest{
    ...
    fun activeTask_displayedOnUi() = runBlockingTest{
        val task = Task(title = "title 1",description = "description 1",isCompleted = false)
    ...
    fun completedTaskDetails_DisplayedInUi() = runBlockingTest{
        ...
        val task = Task(title = "completed Task",description = "description 2",isCompleted = true)
        repository.saveTask(task)
        ...
        onView(withId(R.id.task_detail_description_text)).check(matches(isDisplayed()))
        onView(withId(R.id.task_detail_description_text)).check(matches(withText("description 2")))
        // and make sure the "active" checkbox is shown unchecked
        onView(withId(R.id.task_detail_complete_checkbox)).check(matches(isDisplayed()))
